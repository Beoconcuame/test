...
try:
    ...
    print("CRITICAL: dataset.py not found or GLUEDataset class is missing.")
    sys.exit(1)
...
def prepare_dataloaders(config, tokenizer):
    """
    Prepares and returns DataLoaders for training/validation or evaluation based on the config.
    ...
    Returns:
        tuple: A tuple containing:
            - dict: A dictionary mapping split names ('train', 'val', or 'eval') to DataLoader instances.
            - int: The number of classes for the specified task.

    Raises:
        SystemExit: If the task is invalid, dataset is not found, or dataset is empty.
    """
    task = config['task']
